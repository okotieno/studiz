...
import { InstitutionRequestBackendService } from './institution-request-backend.service';
import {
  QueryOperatorEnum,
...
describe('InstitutionRequestBackendService', () => {
  let service: InstitutionRequestBackendService;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getModelToken(InstitutionRequestModel),
          useValue: modelRepositoryMock,
        },
      ],
    }).compile();

    service = module.get<InstitutionRequestBackendService>(
      InstitutionRequestBackendService
    );
  });
